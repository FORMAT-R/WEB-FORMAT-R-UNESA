...
<div class="page">
  ...
  <nav class="util-nav">
    <div class="util-item" style="display:flex; align-items:center;">
      <a href="{{ route('home') }}#berita" style="font-weight:700; color:var(--red); text-transform:uppercase; letter-spacing:1px; font-size:12px;">&larr; Kembali ke Beranda</a>
    </div>
        ...
        <ul style="display:flex; flex-direction:column; gap:16px;">
          ...
          <li style="border:none; padding:0;">
            <div style="display:flex; flex-direction:column; gap:6px;">
              <a href="{{ route('berita.show', $lb->slug) }}" style="font-weight:600; color:var(--ink); line-height:1.3; font-size:13px;">{{ $lb->title }}</a>
    ...
    <div class="weekly-grid">
      @if(isset($weeklyBerita) && $weeklyBerita->count() > 0)
        ...
        <article class="weekly-card">
          ...
          <a class="jump" href="{{ route('berita.show', $wb->slug ?? $wb->id) }}">Lanjut Baca &rarr;</a>
        </article>
        @endforeach
      @else
...
<script>
  ...
  async function downloadAsPdf() {
    ...
    try {
      ...
      // A4 = 210 x 297 mm; scale to fit width
      const mmWidth  = 210;
