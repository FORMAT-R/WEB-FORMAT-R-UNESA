...
<script>
    function eventForm() {
        return {
            ...
            speakers: {!! json_encode(old('speakers', [['name' => '', 'role' => '', 'topic' => '', 'photo' => null, 'sort_order' => 0]])) !!}.map((s, i) => ({...s, id: s.id || Date.now() + i + 1000})),
            ...
            addSpeaker() { 
                this.speakers.push({id: Date.now(), name: '', role: '', topic: '', photo: null, sort_order: this.speakers.length}); 
            },
