...
<div class="space-y-8" x-data="{
    ...
    <form action="{{ route('admin.departemen.update', $department->id) }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true" class="space-y-8">
        ...
        <section class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 sm:p-8" x-data="{
            prokers: {{ $department->programs->count() > 0 ? $department->programs->map(fn($p) => ['id' => $p->id, 'no' => $p->no, 'name' => $p->name, 'description' => $p->description])->toJson() : "[{ id: Date.now(), no: '01', name: '', description: '' }]" }},
            addProker() {
