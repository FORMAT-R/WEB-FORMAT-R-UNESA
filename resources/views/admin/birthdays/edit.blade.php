...
@section('title', 'Edit Ulang Tahun - FORMAT-R UNESA')

@section('content')
<div class="space-y-8" x-data="{
    form: {
        photoPreview: '{{ $birthday->photo ? Storage::url($birthday->photo) : '' }}'
    },
    isSubmitting: false,
    handlePhotoUpload(e) {
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Data Ulang Tahun</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Perbarui informasi tanggal lahir pengurus.</p>
        </div>
    ...
    <form action="{{ route('admin.ultah.update', $birthday->id) }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <section class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $birthday->name) }}"
                ...
                <div>
                    ...
                    <input type="date" name="birth_date" value="{{ old('birth_date', $birthday->birth_date ? \Carbon\Carbon::parse($birthday->birth_date)->format('Y-m-d') : '') }}" required
                        class="w-full rounded-xl border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white">
                ...
                <div>
                    ...
                    <input type="text" name="department" required value="{{ old('department', $birthday->department) }}" class="w-full rounded-xl border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white" placeholder="Cth: PSDM">
                ...
                <div>
                    ...
                    <input type="text" name="position" value="{{ old('position', $birthday->position) }}"
                        class="w-full rounded-xl border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="message" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Selamat ulang tahun!...">{{ old('message', $birthday->message) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="celebration_status" value="belum_dirayakan" {{ old('celebration_status', $birthday->celebration_status) == 'belum_dirayakan' ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <span class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Belum Dirayakan</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="celebration_status" value="sudah_dirayakan" {{ old('celebration_status', $birthday->celebration_status) == 'sudah_dirayakan' ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
