...
<head>
    ...
    <title>Lupa Kata Sandi - Admin FORMAT-R UNESA</title>
...
<body class="bg-gray-50 text-gray-900 dark:bg-gray-900 dark:text-gray-100 flex items-center justify-center min-h-screen p-4 selection:bg-primary-500 selection:text-white transition-colors duration-200"
    ...
    <div class="glass w-full max-w-md rounded-2xl p-8 relative z-10">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Lupa Sandi?</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Masukkan email Anda untuk menerima tautan reset kata sandi</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.password.email') }}" class="space-y-5">
            ...
            @if (session('status'))
                <div class="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800/50 text-emerald-600 dark:text-emerald-400 text-sm p-4 rounded-lg text-center font-medium">
                    {{ session('status') }}
                </div>
            @endif
        ...
        <div class="mt-8 text-center border-t border-gray-200 dark:border-gray-700 pt-6">
            <a href="{{ route('admin.login') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors gap-2">
